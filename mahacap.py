...
.stMultiselect>div>div{background-color:#1f2937;color:#ffffff;}
...
# -------------------- Admin Panel --------------------
    ...
    with admin_tabs[1]:
        ...
        # ---------------- Other tabs here (full CAP form) -----------------
        with cap_form_tabs[1]:
            st.number_input("Residential Electricity Consumption (kWh/year)", min_value=0, key="res_energy")
            st.number_input("Commercial Electricity Consumption (kWh/year)", min_value=0, key="com_energy")
            st.number_input("Industrial Electricity Consumption (kWh/year)", min_value=0, key="ind_energy")
            st.slider("Renewable Energy Share (%)", 0,100,0, key="renewable_share")
            st.number_input("No. of Energy-Efficient Buildings Certified", min_value=0, key="ee_buildings")
            st.selectbox("Street Lighting Type", ["LED","CFL","Other"], key="street_type")
            st.slider("Street Lighting Coverage (%)",0,100,0, key="street_coverage")
            st.multiselect("Fuel Types Used", ["Coal","Gas","Biomass","Electricity","Petroleum"], key="fuel_types")
            st.number_input("Public Building Energy Consumption (kWh/year)", min_value=0, key="public_energy")
            st.selectbox("Green Building Policies in Place", ["Yes","No"], key="green_policy")

        with cap_form_tabs[2]:
            st.number_input("Total Green Cover Area (ha)", min_value=0, key="green_cover")
            st.number_input("Tree Density (trees/km²)", min_value=0, key="tree_density")
            st.number_input("Protected / Forest Areas within City (ha)", min_value=0, key="protected_areas")
            st.number_input("Wetlands / Water Bodies Coverage (ha)", min_value=0, key="wetlands_area")
            st.text_area("Species Richness (flora & fauna)", key="species_richness")
            st.number_input("Number of Urban Parks / Community Gardens", min_value=0, key="urban_parks")
            st.number_input("Trees Planted per Year", min_value=0, key="afforestation")
            st.selectbox("Biodiversity Management Committees", ["Yes","No"], key="biodiversity_committees")

        with cap_form_tabs[3]:
            st.slider("Modal Share of Public Transport (%)",0,100,0,key="modal_share")
            st.number_input("No. of Electric Vehicles",min_value=0,key="electric_vehicles")
            st.number_input("Length of Bike Lanes (km)",min_value=0,key="bike_lanes")
            st.selectbox("Pedestrian Infrastructure",["Yes","No"],key="pedestrian_infra")
            st.selectbox("Congestion Pricing in Place",["Yes","No"],key="congestion_pricing")
            st.slider("Traffic Emission Reduction (%)",0,100,0,key="traffic_emission_reduction")

        with cap_form_tabs[4]:
            st.number_input("Total Water Consumption (ML/year)",min_value=0,key="water_consumption")
            st.slider("Water Reuse / Recycling (%)",0,100,0,key="water_reuse")
            st.selectbox("Rainwater Harvesting Mandated",["Yes","No"],key="rainwater_harvesting")
            st.number_input("Groundwater Recharge (ML/year)",min_value=0,key="groundwater_recharge")
            st.slider("Water Loss due to Leakage (%)",0,100,0,key="leakage_loss")
            st.number_input("Wastewater Treated (ML/year)",min_value=0,key="wastewater_treatment")

        with cap_form_tabs[5]:
            st.number_input("Municipal Solid Waste Generated (T/year)",min_value=0,key="msw_generated")
            st.slider("Recycling Rate (%)",0,100,0,key="recycling_rate")
            st.slider("Composting Rate (%)",0,100,0,key="composting_rate")
            st.selectbox("Waste-to-Energy Plants",["Yes","No"],key="waste_to_energy")
            st.number_input("Hazardous Waste Generated (T/year)",min_value=0,key="hazardous_waste")
            st.number_input("E-Waste Generated (T/year)",min_value=0,key="e_waste")

        with cap_form_tabs[6]:
            st.number_input("Average Annual Temperature (°C)",key="annual_temp")
            st.number_input("Average Annual Rainfall (mm)",key="avg_rainfall")
            st.text_area("Extreme Climate Events Observed",key="extreme_events")
            st.selectbox("RCP Scenario",["RCP2.6","RCP4.5","RCP6.0","RCP8.5"],key="rcp_scenario")
            st.slider("Projected Temp Increase by 2050 (°C)",0,5,1,key="projected_temp_increase")
