...
def city_page():
    ...
    basic_info_html = f"""..."""  # same structure as in your previous city_page code
    st.markdown(basic_info_html, unsafe_allow_html=True)

    ghg_sectors = ["Energy","Transport","Waste","Water","Buildings","Industry"]
    ghg_values = [city_info.get("GHG", {}).get(s,0) for s in ghg_sectors]
    fig_ghg = px.bar(x=ghg_sectors, y=ghg_values, labels={"x":"Sector","y":"tCO2e"}, title=f"{selected_city} GHG Emissions by Sector", template="plotly_dark")
    ...
    fig_rcp.update_layout(template="plotly_dark", title=f"{selected_city} RCP Scenario Projections", xaxis_title="Year", yaxis_title="Temp Rise (°C)")
    st.plotly_chart(fig_rcp, use_container_width=True)

    st.markdown(f"<div style='position:fixed; bottom:10px; center:10px; color:#aaaaaa; font-size:12px;'>Last Updated: {last_updated()}</div>", unsafe_allow_html=True)
