...
def sidebar_section():
    ...
    st.sidebar.markdown('<div class="sidebar-logo"><img src="https://github.com/eintrusts/CAP/blob/main/EinTrust%20%20(2).png?raw=true"></div>', unsafe_allow_html=True)
    ...
    st.sidebar.markdown('<div class="sidebar-footer"><strong>© EinTrust 2025</strong><br>All Rights Reserved</div>', unsafe_allow_html=True)
...
def city_page():
    ...
    cap_link = city_info.get("CAP_File", "")
    if cap_link:
        st.markdown(f"[Download CAP]({cap_link})")
...
def admin_panel():
    ...
    with admin_tabs[0]:
        ...
        population = st.number_input("Population", min_value=0, value=st.session_state.city_data.get(city_select, {}).get("Basic Info", {}).get("Population",0), key="population")
        area = st.number_input("Area (sq km)", min_value=0, value=st.session_state.city_data.get(city_select, {}).get("Basic Info", {}).get("Area",0), key="area")
        gdp = st.number_input("GDP (₹ Crores)", min_value=0, value=st.session_state.city_data.get(city_select, {}).get("Basic Info", {}).get("GDP",0), key="gdp")
        ...
        cap_file = st.text_input("CAP File Link (if available)", value=st.session_state.city_data.get(city_select, {}).get("CAP_File",""), key="cap_file")
    ...
    # --- Generate CAP (All tabs fully restored) ---
    with admin_tabs[1]:
        st.subheader("Generate CAP")
        cap_city = st.selectbox("Select City", cities, key="cap_city_generate")
        city_info = st.session_state.city_data.get(cap_city, {})
        ...
        # --- Tab 1: Basic Info ---
        with cap_tabs[0]:
            st.write("City Basic Info")
            st.text_input("Population", value=city_info.get("Basic Info", {}).get("Population",""))
            st.text_input("Area", value=city_info.get("Basic Info", {}).get("Area",""))
            st.text_input("GDP", value=city_info.get("Basic Info", {}).get("GDP",""))

        # --- Tab 2: Energy & Buildings ---
        with cap_tabs[1]:
            st.write("Energy & Buildings")
            st.text_area("Energy Sector Details", value=city_info.get("Energy",""))

        # --- Tab 3: Green Cover & Biodiversity ---
        with cap_tabs[2]:
            st.write("Green Cover & Biodiversity")
            st.text_area("Green Cover Details", value=city_info.get("Green_Cover",""))

        # --- Tab 4: Sustainable Mobility ---
        with cap_tabs[3]:
            st.write("Sustainable Mobility")
            st.text_area("Mobility Details", value=city_info.get("Mobility",""))

        # --- Tab 5: Water Resources ---
        with cap_tabs[4]:
            st.write("Water Resources")
            st.text_area("Water Details", value=city_info.get("Water",""))

        # --- Tab 6: Waste Management ---
        with cap_tabs[5]:
            st.write("Waste Management")
            st.text_area("Waste Details", value=city_info.get("Waste",""))

        # --- Tab 7: Climate Data ---
        with cap_tabs[6]:
            st.write("Climate Data / RCP Scenario")
            st.text_area("Climate Scenario", value=city_info.get("Climate_Data",""))
