...
def admin_panel():
    ...
    # --- Add/Update City ---
    # -------------------- Admin Panel: Add/Update City --------------------
def admin_add_update_city():
    if 'current_admin_tab' not in st.session_state:
        st.session_state.current_admin_tab = 0  # default Add/Update City tab
    ...
    st.text_input("District", value=district, key="district", disabled=True)
    ...
    if st.button("Add/Update City"):
        st.session_state.city_data[city_select] = {
            "District":district if district else city_info.get("District",""),
